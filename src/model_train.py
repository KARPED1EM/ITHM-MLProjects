...
def load_data():
    df_train = pd.read_csv('data/p_train.csv')
    df_test = pd.read_csv('data/p_test.csv')
...
def model_train(model, train_data_loader, val_data_loader):
    ...
    torch.save(best_model, 'model/p_best_model.pth')
    print(f"Training finished. Best Val F1: {best_f1:.4f}")
    print('*' * 20)

def model_test(model, test_data_loader):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model.load_state_dict(torch.load('model/p_best_model.pth'))
    model = model.to(device)
